...
GoDo is MIT (see [LICENSE](./LICENSE)). The binaries shipped on GitHub Releases
statically link the libraries below, so their notices travel with them.
...
WebAssembly runtime. Used to run plugins.
...
Licensed under the MIT License, with portions under the Apache License,
Version 2.0. See the module's `LICENSE` and `NOTICE` files.
